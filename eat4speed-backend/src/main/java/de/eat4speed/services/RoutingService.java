    ...
    public JSONArray get_best_Route() throws Exception {
        ...
        for (int i = 0; i < legs.length(); i++) {
            ...
            for (int j = 0; j < waypoints.getJSONObject(legs.getJSONObject(i).getInt("to_waypoint_index")).getJSONArray("actions").length(); j++) {
                beschreibung.append(waypoints.getJSONObject(legs.getJSONObject(i).getInt("to_waypoint_index")).getJSONArray("actions").getJSONObject(j).get("type")).append(" ");
                try {
                    beschreibung.append(waypoints.getJSONObject(legs.getJSONObject(i).getInt("to_waypoint_index")).getJSONArray("actions").getJSONObject(j).get("shipment_id")).append(" ");
                } catch (Exception e) {
            ...
            list_waypoints.put(new JSONObject()
                    ...
                    .put("entfernung", legs.getJSONObject(i).get("distance") + "m")
